...
pub fn start(relays: RelayBank<RELAY_COUNT>, spawner: Spawner) {
    spawner.spawn(relay_task(relays).unwrap());
}

/// Create a command handle for this board's relay controller.
///
/// The handle can be created before the relay task starts. Commands wait in
/// the channel until the core-1 relay task receives them.
pub const fn control() -> RelayControl {
